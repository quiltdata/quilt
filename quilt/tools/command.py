...
def get_registry_url():
    ...
    if _registry_url is not None:
        # Treat '' as the default URL.
        return _registry_url or DEFAULT_REGISTRY_URL

    # Env variable; '' can be used to override a custom URL in the config.
    _registry_url = os.environ.get('QUILT_PKG_URL')
    if _registry_url is not None:
        return _registry_url

    # Config file (generated by `quilt config`).
    cfg = _load_config()
    _registry_url = cfg.get('registry_url', '')
...
def config():
    ...
    if answer:
        ...
        _registry_url = urlunparse(url)
    else:
        _registry_url = ''  # When saving the config, store '' instead of the actual URL in case we ever change it.

    cfg = _load_config()
    cfg['registry_url'] = _registry_url
    _save_config(cfg)
