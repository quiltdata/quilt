...
def _repair_package(packagename):
    """
    fixes null metadata['q_path'] instances for packagename 'user/foo'

    When developing the TensorFlow branch, initial development created
    packages with file objects missing metadata['q_path'].  This has been
    fixed, but local PackageStores may be corrupted in a way that prevents
    loading builds that have the corrupted data, now that `q_path` is
    required.

    Running this repairs the PackageStore for the specific package by
    converting the module path to a valid 'q_path'.
    """
    import json
    from . import store

    def fix_null_q_path(tree, path=[]):
        result = False

        if 'children' in tree:
            for child in tree['children']:
                result = fix_null_q_path(tree['children'][child], path=path + [child])
        metadata = tree.get('metadata')

        if metadata and metadata['q_target'] == 'file' and metadata['q_path'] is None:
            metadata['q_path'] = '/'.join(path)
            print("Fixed {} with {}".format(path, metadata['q_path']))
            return True
        return result

    user, package = packagename.split('/')
    packagestore = store.PackageStore()
    packagestore.check_name(user, package)

    path = pathlib.Path(packagestore.package_path(user, package))

    if not path.is_dir():
        raise CommandException("Error: {!r} is not a directory".format(str(path)))

    # Collect objects from all instances for potential cleanup
    contents_path = path / store.Package.CONTENTS_DIR
    for f in contents_path.glob('*'):
        print("Scanning {}".format(str(f)))
        obj = json.loads(f.read_text())
        fixed = fix_null_q_path(obj)
        if fixed:
