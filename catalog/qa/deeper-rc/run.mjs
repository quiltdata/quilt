...
const finalPackageName = env(
  'QA_PACKAGE_NAME',
  `qa/nightly-${normalizedVersion}-rc-deeper-qa`,
)
...
const outputRoot = path.resolve(
  env('QA_OUTPUT_ROOT', path.join(process.cwd(), '.playwright-mcp')),
)
const profileDir = path.resolve(
  env('QA_PROFILE_DIR', path.join(outputRoot, 'nightly-visible-profile')),
)
const authMode = env('QA_AUTH_MODE', 'profile').toLowerCase()
const defaultEncryptedProfilePath = path.join(
  __dirname,
  'secrets',
  'deeper-rc-browser-profile.tgz.gpg',
)
const profileArchivePath = env('QA_BROWSER_PROFILE_TGZ')
const resolvedProfileArchivePath = profileArchivePath
  ? path.resolve(profileArchivePath)
  : ''
const encryptedProfileArchivePath = path.resolve(
  env('QA_BROWSER_PROFILE_GPG', defaultEncryptedProfilePath),
)
const profileArchiveB64 = env(
  'QUILT_QA_BROWSER_PROFILE_TGZ_B64',
  env('QUILT_QA_BROWSER_PROFILE_B64'),
)
...
const profileB64ExportPath = path.resolve(
  env(
    'QA_PROFILE_B64_EXPORT_PATH',
    profileExportPath.endsWith('.b64') ? profileExportPath : `${profileExportPath}.b64`,
  ),
)
const profileGpgExportPath = path.resolve(
  env('QA_PROFILE_GPG_EXPORT_PATH', defaultEncryptedProfilePath),
)
const profileEncryptPassphrase = env('QA_PROFILE_ENCRYPT_PASSPHRASE')
const writeLegacyB64 = ['1', 'true', 'yes'].includes(
  env('QA_PROFILE_WRITE_B64', 'true').toLowerCase(),
)
...
const athenaEvidence = {
  mcp: {
    status: 'blocked',
    reason:
      "MCP athena_query tool failed before this runner started: Failed to resolve dependency 'athena'.",
  },
}
...
function assertSupportedAuthMode() {
  ...
  if (!supported.has(authMode)) {
    throw new Error(
      `Unsupported QA_AUTH_MODE=${authMode}; expected profile, credentials, or microsoft-interactive`,
    )
  }
  if (authMode === 'microsoft-interactive' && process.env.GITHUB_ACTIONS) {
    throw new Error(
      'QA_AUTH_MODE=microsoft-interactive is local-only and cannot complete in GitHub Actions',
    )
  }
}
...
function runGpg(args) {
  try {
    ...
    const output = [error.stdout, error.stderr]
      ...
      .trim()
    throw new Error(
      `gpg ${args.filter((arg) => arg !== profileEncryptPassphrase).join(' ')} failed${output ? `: ${output}` : ''}`,
    )
  }
}
...
function restoreBrowserProfileFromArchive(archivePath) {
  if (!fs.existsSync(archivePath)) {
    throw new Error(
      `QA_BROWSER_PROFILE_TGZ points to missing profile archive: ${archivePath}`,
    )
  }
  const stats = fs.statSync(archivePath)
  if (!stats.size) {
    throw new Error(
      `QA_BROWSER_PROFILE_TGZ points to an empty profile archive: ${archivePath}`,
    )
  }
...
function restoreBrowserProfileFromEnv() {
  ...
  if (!decoded.length) {
    throw new Error(
      `${profileArchiveSecretName || 'QUILT_QA_BROWSER_PROFILE_TGZ_B64'} decoded to an empty profile archive`,
    )
  }
  ...
  console.log(
    `Restored browser profile from ${profileArchiveSecretName} into ${profileDir}`,
  )
  return true
}
...
function printProfileTransportInstructions(tgzPath, b64Path) {
  ...
  console.log('')
  console.log(
    'Commit only the encrypted .tgz.gpg profile. Do not commit the plaintext .tgz profile.',
  )
  if (b64Path) {
    console.log(
...
function createBrowserProfileArchive() {
  const tgzPath = profileExportPath.endsWith('.b64')
    ? profileExportPath.replace(/\.b64$/, '')
    : profileExportPath
  const excludes = [
    './Default/Cache',
    './Default/Code Cache',
...
async function run(name, fn) {
  ...
  try {
    ...
    const screenshot = page
      ? path.join(screenshotsDir, `${safeName(name)}-failed.png`)
      : null
    if (page && screenshot) {
...
async function capture(name, routeOrUrl, targetPage = page) {
  ...
  const diagnostics = await targetPage.evaluate(() => {
    ...
    const controls = Array.from(
      document.querySelectorAll(
        "button, a, input, textarea, select, [role='button'], [role='tab'], [role='link']",
      ),
    )
    ...
    const buttonsWithoutNames = controls
      .filter(
        (el) => /^(BUTTON|A)$/.test(el.tagName) || el.getAttribute('role') === 'button',
      )
      .filter((el) => {
  ...
  if (diagnostics.overflowX > 8) {
    addFinding(
      ...
      name,
      'visual',
      `Horizontal overflow detected (${diagnostics.overflowX}px beyond viewport).`,
      {
  ...
  if (diagnostics.accessibility.unlabeledInputs.length) {
    addFinding(
      ...
      name,
      'accessibility',
      'Visible form controls without accessible labels were found.',
      {
        evidence: recordPage.screenshot,
  ...
  if (diagnostics.accessibility.buttonsWithoutNames.length) {
    addFinding(
      ...
      name,
      'accessibility',
      'Buttons or links without accessible names were found.',
      {
        evidence: recordPage.screenshot,
...
async function authenticateCatalog() {
  ...
  if (authMode === 'credentials') {
    if (!qaUsername || !qaPassword) {
      throw new Error(
        'QA_AUTH_MODE=credentials requires legacy QUILT_QA_USERNAME and QUILT_QA_PASSWORD',
      )
    }
...
async function runMicrosoftInteractiveBootstrap() {
  ...
  console.log(
    'Complete Microsoft federated login in the browser window. The runner will wait for the Quilt auth token.',
  )
  ...
  if (!authToken) {
    ...
    await page
      ...
      .catch(() => {
        console.log(
          'Click the Sign in with Microsoft button manually if it was not opened automatically.',
        )
      })
    await page.waitForFunction(() => Boolean(localStorage.getItem('TOKENS')), null, {
...
async function getAwsCredentials() {
  ...
  if (!credentialResult.ok) {
    throw new Error(
      `Credential endpoint returned ${credentialResult.status}: ${credentialResult.text.slice(0, 300)}`,
    )
  }
...
async function graphql(query, variables = {}) {
  const result = await page.evaluate(
    async ({ registryUrl, authToken, query, variables }) => {
...
async function putTextObject(key, content, contentType = 'text/plain; charset=utf-8') {
  await s3
    .putObject({ Bucket: bucket, Key: key, Body: content, ContentType: contentType })
    .promise()
  resources.push({
    type: 's3-object',
    bucket,
    key,
    cleanup: 'left-in-place-if-package-source',
  })
...
async function runAdminCrud() {
  ...
  try {
    ...
    assert(
      policyResult.__typename === 'Policy',
      `Policy create returned ${JSON.stringify(policyResult)}`,
    )
    ...
    assert(
      roleResult.__typename === 'RoleCreateSuccess',
      `Role create returned ${JSON.stringify(roleResult)}`,
    )
    ...
    assert(
      updatePolicy.policyUpdate.__typename === 'Policy',
      `Policy update returned ${JSON.stringify(updatePolicy.policyUpdate)}`,
    )
    ...
    const updateRole = await graphql(
      ...
      {
        id: created.roleId,
        input: { name: roleNameUpdated, policies: [created.policyId] },
      },
    )
    assert(
      updateRole.roleUpdate.__typename === 'RoleUpdateSuccess',
      `Role update returned ${JSON.stringify(updateRole.roleUpdate)}`,
    )
    ...
    assert(
      ...
      `User create returned ${JSON.stringify(userResult)}`,
    )
    ...
    assert(
      updateUser.admin.user.mutate.setEmail.__typename === 'User',
      `User update returned ${JSON.stringify(updateUser.admin.user.mutate.setEmail)}`,
    )

    adminCrud.created = { policyTitle, roleName, userName }
    adminCrud.updated = {
      policyTitle: policyTitleUpdated,
    ...
    if (created.userName) {
      try {
        ...
        cleanup.push({
          ...
          name: created.userName,
          status:
            del.admin.user.mutate.delete.__typename === 'OperationSuccess'
              ? 'deleted'
              : 'delete-requested',
          result: del.admin.user.mutate.delete,
        })
      } catch (error) {
        cleanup.push({
          type: 'user',
...
async function runPersonaMatrix() {
  ...
  const probeRoles = Array.from(new Set(preferred.concat(availableRoles)))
  ...
  for (const role of probeRoles) {
    try {
      ...
      const entry = {
        role,
        accessDenied: /Access Denied|don't have access|not authorized/i.test(
          `${overview.text} ${tree.text} ${packages.text}`,
        ),
        hasUpload: /Add files|Create package|Upload files/i.test(tree.text),
        canSeePackages:
          /package|packages|Create new package|No packages/i.test(packages.text) &&
          !/Access Denied/i.test(packages.text),
      }
      probes.push(entry)
  ...
  if (!readWrite)
    addBlocker(
      'Persona matrix',
      'No assigned role produced a clear read-write persona during role switching.',
      { roles: probeRoles },
    )
  if (!readOnly)
    addBlocker(
      'Persona matrix',
      'No assigned role produced a clear read-only persona during role switching.',
      { roles: probeRoles },
    )
  if (!restricted)
    addBlocker(
      'Persona matrix',
      'No assigned role produced a restricted persona during role switching.',
      { roles: probeRoles },
    )
  personaMatrix.push(
    {
      persona: 'read-write',
      role: readWrite && readWrite.role,
      covered: Boolean(readWrite),
    },
    { persona: 'read-only', role: readOnly && readOnly.role, covered: Boolean(readOnly) },
    {
      persona: 'restricted',
      role: restricted && restricted.role,
      covered: Boolean(restricted),
    },
    anonymous,
  )
  ...
  return {
    roles: probeRoles,
    readWrite: readWrite && readWrite.role,
    readOnly: readOnly && readOnly.role,
    restricted: restricted && restricted.role,
  }
}
...
async function runAnonymousChecks() {
  ...
  try {
    ...
    const gatedAdmin =
      /sign in|login|not authorized|authorization|authenticate|SIGN IN WITH/i.test(
        admin.text,
      )
    const bucketNotAdmin = !/Add files|Create package|Users and roles|Admin/i.test(
      bucketPage.text,
    )
    personaMatrix.push({
      persona: 'anonymous',
...
async function runPackageLifecycle() {
  ...
  const initialEntries = [
    await uploadLifecycleEntry(readmeFile, `${base}/initial/readme.md`, 'README.md', {
      stage: 'initial',
    }),
    await uploadLifecycleEntry(
      dataFile,
      `${base}/initial/data-file.txt`,
      'nested/files/data file.txt',
      { stage: 'initial' },
    ),
    await uploadLifecycleEntry(
      oddFile,
      `${base}/initial/odd-file.txt`,
      'nested odd/odd-file_[1].txt',
      { oddFilename: true },
    ),
  ]
  const rev1 = await packageConstruct(
    packageName,
    initialEntries,
    `e2e lifecycle initial revision (${runId})`,
    {
      qaRunId: runId,
      qaStage: 'initial',
    },
  ...
  const revisedEntries = initialEntries.concat([
    await uploadLifecycleEntry(
      revFile,
      `${base}/revision-2/extra.csv`,
      'revision-2/extra.csv',
      { stage: 'revision-2' },
    ),
  ])
  const rev2 = await packageConstruct(
    packageName,
    revisedEntries,
    `e2e lifecycle revised package (${runId})`,
    {
      qaRunId: runId,
      qaStage: 'revision-2',
    },
  ...
  lifecycle.leftInPlace = {
    reason:
      'Package revisions require their backing S3 package source objects to remain addressable for inspection.',
    packageName,
  ...
  const detail = await capture(
    'lifecycle-package-detail',
    `/b/${bucket}/packages/${packageName}`,
  )
  assert(
    /README|nested|revision-2|Get package|Download|quilt3|Files|Entries/i.test(
      detail.text,
    ),
    'Lifecycle package detail did not expose expected files/get affordance',
  )
  const revisions = await capture(
    'lifecycle-package-revisions',
    `/b/${bucket}/packages/${packageName}/revisions`,
  )
  assert(
    revisions.text.includes(rev1.slice(0, 8)) ||
      revisions.text.includes(rev2.slice(0, 8)) ||
      /revision|latest|hash/i.test(revisions.text),
    'Lifecycle package revisions UI did not show revision history',
  )
...
async function runUploadDownloadCleanup() {
  ...
  fs.writeFileSync(
    localFile,
    `Quilt nightly ${expectedVersion} deeper QA\nrun=${runId}\nbucket=${bucket}\n`,
  )
...
async function fillSearchInput(value) {
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    if (disabled || ['checkbox', 'radio', 'hidden', 'submit', 'button'].includes(type))
      continue
...
async function runSearchAndFiltering() {
  ...
  searchEvidence.global = {
    url: page.url(),
    fill: globalFill,
    textStart: globalAfter.text.slice(0, 500),
  }
  assert(
    /search|result|bucket|quilt-example/i.test(`${global.text} ${globalAfter.text}`),
    'Global search did not render recognizable results or empty state',
  )
  ...
  searchEvidence.packages = {
    url: page.url(),
    fill: packageFill,
    textStart: packagesAfter.text.slice(0, 500),
  }
  assert(
    /package|filter|search|qa|No packages|result/i.test(
      `${packages.text} ${packagesAfter.text}`,
    ),
    'Package search/filter UI did not render recognizable state',
  )
  ...
  searchEvidence.empty = {
    url: page.url(),
    fill: treeFill,
    textStart: empty.text.slice(0, 500),
  }
...
async function startAthena(sql, workgroup) {
  ...
  if (execution.Status.State === 'SUCCEEDED') {
    const result = await athena
      .getQueryResults({ QueryExecutionId: executionId, MaxResults: 10 })
      .promise()
...
async function runAthenaChecks() {
  ...
  if (!workgroup) {
    addBlocker(
      'Athena execution',
      'No Athena workgroup was listable from the temporary catalog credentials.',
      {
  ...
  try {
    ...
    assert(
      ...
      `Expected SELECT 1 to succeed, got ${valid.state}: ${valid.reason || ''}`,
    )
  } catch (error) {
    ...
    addFinding(
      ...
      'Athena execution',
      'functional',
      `SELECT 1 could not be executed successfully: ${error.message}`,
      {
        evidence: 'screenshots/athena-ui.png',
      },
    )
  ...
  try {
    const invalid = await startAthena(
      'SELECT * FROM definitely_missing_e2e_qa_table',
      workgroup,
    )
    athenaEvidence.invalid = invalid
    assert(
      invalid.state === 'FAILED',
      `Expected invalid SQL to fail, got ${invalid.state}`,
    )
  } catch (error) {
    athenaEvidence.invalid = {
      status: 'error-during-invalid-query',
      error: error.message,
    }
    addFinding(
      'low',
      'Athena invalid SQL',
      'diagnostic',
      `Invalid SQL path could not be verified cleanly: ${error.message}`,
      {
        evidence: 'screenshots/athena-ui.png',
      },
    )
...
async function runReleaseValidation() {
  ...
  if (
    !release.displayedVersion ||
    !versionRegex(expectedVersion).test(release.displayedVersion)
  ) {
    addFinding(
      'medium',
      'Release/version display',
      'release validation',
      `The nightly footer reports ${release.displayedVersion || 'no visible version'}, which does not visibly identify the expected ${expectedVersion} release candidate.`,
      {
...
- Area: ${f.area}
- Type: ${f.type}
- Evidence: \`${f.evidence || 'data/qa-summary.json'}\`${
            f.details
              ? `
...
async function publishFinalPackage() {
  fs.copyFileSync(
    __filename,
    path.join(outDir, `automation-nightly-${normalizedVersion}-deeper-qa.mjs`),
  )
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(summaryObject(null), null, 2),
  )
...
;(async () => {
  ...
  if (consoleErrors.length) {
    addFinding(
      ...
      'Browser console',
      'diagnostic',
      `${consoleErrors.length} console error(s) observed during QA run.`,
      {
  ...
  if (requestFailures.length) {
    addFinding(
      ...
      'Network',
      'diagnostic',
      `${requestFailures.length} failed network request(s) observed during QA run.`,
      {
        evidence: 'data/qa-summary.json',
