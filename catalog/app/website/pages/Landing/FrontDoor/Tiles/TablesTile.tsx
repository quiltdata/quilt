...
import { queriesAthena } from 'constants/routes'
...
// Deep-links a Tabulator table into the query console, matching the
// `?table=<name>` affordance from the Overview v2 Tabulator chips. The console
// is workspace-global now, so the bucket rides along as a param rather than in
// the path (the old `/b/:bucket/queries/athena` route only redirects here).
export function tableHref(bucket: string, table: string): string {
  return queriesAthena.url({ bucket, table })
}
