...
function getMetadataFields(metadata: ParquetMetadata | H5adMetadata | PackageMetadata) {
  return Object.entries(FIELDS_MAP).reduce(
    (memo: MetadataField[], [key, { title, RenderValue }]) => {
      const value =
        metadata[key as keyof (ParquetMetadata | H5adMetadata | PackageMetadata)]
      return value != null ? [...memo, { key, title, value, RenderValue }] : memo
    },
    [],
  )
}
...
export default function Metadata({ className, metadata }: RenderMetaProps) {
  ...
  const fields = React.useMemo(() => getMetadataFields(metadata), [metadata])
  return (
    <M.Table className={cx(className, classes.root)} size="small">
      ...
      <M.TableBody>
        {fields.map(({ key, title, value, RenderValue }) => (
          <M.TableRow key={key}>
