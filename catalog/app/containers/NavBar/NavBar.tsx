  ...
  ({ children, ...props }: ItemProps, ref: React.Ref<any>) => {
    ...
    return (
      <M.MenuItem
        ...
        component={props.to ? RRDom.Link : props.href ? 'a' : undefined}
        ref={ref}
        {...props}
      >
...
function UserDropdown() {
  ...
  const isProfile = !!RRDom.useRouteMatch({ path: paths.profile, exact: true })
  const isAdmin = !!RRDom.useRouteMatch(paths.admin)
...
const NavLink = React.forwardRef((props: NavLinkProps, ref: React.Ref<unknown>) => {
  const isActive = !!RRDom.useRouteMatch({ path: props.path, exact: true })
  return (
    <M.Box
...
export function NavBar() {
  ...
  const isSignIn = !!RRDom.useRouteMatch({ path: paths.signIn, exact: true })
