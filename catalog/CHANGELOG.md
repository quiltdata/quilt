...
- [Fixed] Package revise dialog: when a package's existing contents cannot be loaded, pushing a revision over it is now blocked and the reason is shown, instead of silently replacing the latest revision with only the newly added files and dropping everything that failed to load. This covers a failed request and also a partial response, which previously resolved as success and could drop the package metadata or substitute the bucket's default workflow without any visible error. Staged files are kept, and pushing under a name that does not exist yet is still allowed ([#5165](https://github.com/quiltdata/quilt/pull/5165))
