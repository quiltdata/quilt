...
- [Fixed] Package revise dialog: a failed manifest fetch (e.g. a transient backend error) now shows an error instead of silently rendering an empty form whose submission replaced the latest revision with only the newly added files, dropping all pre-existing files and package metadata ([#5165](https://github.com/quiltdata/quilt/pull/5165))
