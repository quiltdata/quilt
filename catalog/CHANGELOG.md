...
- [Fixed] Admin Users table: for a service user (the stack-managed `_canary` account) the Enabled toggle is disabled, the email is no longer click-to-edit, and the Delete action is hidden. The registry refuses all three mutations, so those controls could only ever produce an error. The role and admin controls are unchanged, continuing to follow the server's `isRoleAssignmentDisabled` / `isAdminAssignmentDisabled` flags ([#5183](https://github.com/quiltdata/quilt/pull/5183))
