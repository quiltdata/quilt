...
- [Added] Admin Buckets: an always-visible status box on the add/edit forms surfaces a bucket's live S3 versioning state — advisory only, never blocking submission: green when versioning is enabled; a warning when it's suspended or the bucket is unversioned (with a link to Quilt's bucket-versioning docs); and a not-found / access-denied pre-flight. A Refresh re-checks over the network (bypassing the cache) ([#5090](https://github.com/quiltdata/quilt/pull/5090))
