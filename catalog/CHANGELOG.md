...
- [Changed] Search: the unexpected, query-syntax and timeout error screens are separate components rather than one component behind a `kind` prop, so each offers only the recovery actions it actually has. Nothing they render changes ([#5238](https://github.com/quiltdata/quilt/pull/5238))
