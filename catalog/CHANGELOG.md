...
- [Added] Bucket Overview v2, opt-in per bucket via `ui.blocks.overviewV2`: a denser header (name + navigational stats incl. a Tabulator-tables count, `+ Create package`, and a collapsible README that fades out instead of hard-cropping), an inline Qurator chat that hands its conversation off to the global sidebar on navigation, a config-driven Tabulator tables section (click-to-expand schemas, zero Athena on load), a latest-packages list, and a `?table=` deep-link from the Overview into the Athena Queries editor. Queries-bound surfaces respect `ui.nav.queries`; sections share a common `SectionCard`/`SectionHeader` ([#4995](https://github.com/quiltdata/quilt/pull/4995))
