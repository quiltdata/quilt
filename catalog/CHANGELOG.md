...
- [Changed] Qurator issues its model call through the platform's inference relay instead of calling Bedrock from the browser, so a deployment can route inference through a customer-operated AI gateway whose credential never reaches the client. ([#5334](https://github.com/quiltdata/quilt/pull/5334))
