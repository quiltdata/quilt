...
- [Changed] Object delete writes an S3 delete marker instead of destroying a specific object version, so prior versions are retained and stay available in the object's version history — which is what the 1.63.0 release notes have always described. `DeleteDialog` re-resolves the handles it is given through `headObject`, which returns a `VersionId` on a versioned bucket whether or not one was requested, so all three delete entrypoints (file viewer, directory-listing row action, and bulk "Delete selected") previously destroyed a version. Permanently erasing a version now requires an admin-attached custom policy granting `s3:DeleteObjectVersion`, and the example policies in the docs present that action as an explicit opt-in ([#5161](https://github.com/quiltdata/quilt/pull/5161))
