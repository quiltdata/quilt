...
- [Fixed] Package revise dialog: when a package's existing contents fail to load — including a partial response, which previously resolved as success — pushing a revision over it is now blocked with the reason shown, instead of silently replacing the latest revision with only the newly added files. Staged files are kept, and pushing under a name that does not exist yet is still allowed ([#5165](https://github.com/quiltdata/quilt/pull/5165))
