...
- [Changed] Front-door v2 (`frontDoorV2`): the Buckets tile now links to a dedicated `/buckets` page (wrapped in the standard layout), the tile rows show bucket title + `s3://` path with a folder icon and always-present browse affordance, and the example-query chips are derived from real catalog data (recently-revised package names, bucket tags and titles) with a generic fallback when data is sparse.
- [Added] Dev-flagged front-door v2 landing surface for evaluation builds: server-backed recently-revised Recent packages tile, bounded/expandable Buckets tile with relevance/recent/A-Z sorting, real Tabulator Tables rollup tile, a compact footer, and app-shell integration that surfaces the unified search as a navbar omnisearch and moves the Qurator trigger to a rightmost navbar button (all gated behind `frontDoorV2`).
