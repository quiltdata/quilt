...
- [Changed] Data products (preview): the `data-products` surfaces are rebuilt on the local volume model — a product is a virtual volume defined by a SQL query over buckets a workspace already holds, offered through a local exchange and read by minting credentials against a capture — replacing the external-catalog read shape, on example data until a registry serves the API ([#XXXX](https://github.com/quiltdata/quilt/pull/XXXX))
