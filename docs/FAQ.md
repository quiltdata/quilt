...
1. If you are using Quilt Catalog 1.51 (released Feb 2024), you can enable CHUNKED_CHECKSUMS which will calculate the checksums in parallel, or reuse them if already existing in S3. Parallel checksums are also available by default in `quilt3` v6 or later (pre-released Feb 2024).
