...
* [Fixed] `quilt3.admin.roles.get()` and `quilt3.admin.policies.get()` no longer send a non-UUID argument to the by-ID query, where the registry could fail with a server error. A name or title is now recognized before any query is issued and resolved through the by-name/by-title lookup alone ([#5175](https://github.com/quiltdata/quilt/issues/5175))
