...
* [Fixed] A cached manifest that cannot be parsed (e.g. truncated by a pre-8.0 concurrent download) is evicted and re-downloaded once instead of failing every future `Package.browse()` of that revision with a raw JSON error ([#5217](https://github.com/quiltdata/quilt/pull/5217))
* [Fixed] `Package.browse()` of a manifest that cannot be verified because an entry lacks `hash` or `size` (typically written by a non-quilt3 producer) now raises `PackageException` naming the package and revision and stating that verification is the failing step, instead of a bare error naming only a physical key ([#5217](https://github.com/quiltdata/quilt/pull/5217))
