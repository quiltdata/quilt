...
* [Changed] Manifest top-hash verification is unconditional: no keyword, environment variable, or setting skips it (`QUILT_DISABLE_CACHE` bypasses the manifest cache, not the check). A `Package.browse()` or `Package.install()` call that previously succeeded now raises `PackageException` if the manifest does not hash to the revision it was resolved to, so a corrupted cache entry or an externally modified manifest becomes a hard failure instead of silently mismatched data. Verification is CPU-only and issues no additional requests, adding roughly 30% to manifest load time — visible only when nothing else dominates, such as a warm cache or a local registry ([#5184](https://github.com/quiltdata/quilt/pull/5184))
