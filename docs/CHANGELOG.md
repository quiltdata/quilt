...
* [Fixed] API keys set with `login_with_api_key()` are scoped to the currently resolved registry URL, so switching registries in-process cannot send one registry's key to another; `clear_api_key()` clears only the current registry's key, while `logout()` continues to clear all in-memory keys ([#5248](https://github.com/quiltdata/quilt/pull/5248))
