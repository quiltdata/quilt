...
* [Fixed] Downloading a specific object version now probes that version when choosing between the signed and unsigned S3 client, so a public bucket that grants `s3:GetObject` but not `s3:GetObjectVersion` no longer selects a client that fails on the subsequent `get_object`. The signed/unsigned decision is also cached separately for versioned and unversioned reads of the same bucket ([#4639](https://github.com/quiltdata/quilt/issues/4639))
