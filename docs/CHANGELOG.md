...
* [Fixed] `Bucket.fetch()` on a directory no longer raises `ValueError: Invalid relative path: ''` when the listing contains zero-byte directory markers, including the prefix itself; the markers are skipped and the real objects download. Genuine zero-byte files still download, and a marker with a non-zero size warns ([#4641](https://github.com/quiltdata/quilt/issues/4641))
