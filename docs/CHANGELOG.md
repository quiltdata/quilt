...
* [Added] `quilt3.admin.RoleOwnsDataProductsError` — `admin.roles.delete()` raises it when the registry refuses to delete a role that owns data products. Earlier clients, whose generated result union does not carry the type, fail that response with an unhandled pydantic `ValidationError` ([#5299](https://github.com/quiltdata/quilt/pull/5299))
...
* [Fixed] A cached manifest that cannot be parsed (e.g. truncated by a pre-8.0 concurrent download) is evicted and re-downloaded once instead of failing every future `Package.browse()` of that revision with a raw JSON error ([#5218](https://github.com/quiltdata/quilt/pull/5218))
* [Fixed] `Package.browse()` of a manifest that cannot be verified because an entry lacks `hash` or `size` (typically written by a non-quilt3 producer) now raises `QuiltException` naming the package and revision and stating that verification is the failing step — so the CLI prints an error instead of a traceback — and evicts a cached copy so a later server-side fix is picked up, instead of a bare error naming only a physical key ([#5218](https://github.com/quiltdata/quilt/pull/5218))
