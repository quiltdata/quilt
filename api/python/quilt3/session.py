...
def _load_auth():
    if AUTH_PATH.exists():
        with open(AUTH_PATH, encoding='utf8') as fd:
            return json.load(fd)
    return {}
...
def _save_auth(cfg):
    ...
    with open(AUTH_PATH, 'w', encoding='utf8') as fd:
...
def _load_credentials():
    if CREDENTIALS_PATH.exists():
        with open(CREDENTIALS_PATH, encoding='utf8') as fd:
            return json.load(fd)
    return {}
...
def _save_credentials(creds):
    BASE_PATH.mkdir(parents=True, exist_ok=True)
    with open(CREDENTIALS_PATH, 'w', encoding='utf8') as fd:
...
def open_url(url):
    try:
        if sys.platform == 'win32':
            ...
            with open(os.devnull, 'r+', encoding='utf8') as null:
