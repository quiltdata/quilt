...
class PackageTest(QuiltTestCase):
    ...
    def test_build_default_registry(self):
        ...
        for suffix in ('suffix1', 'suffix2'):
            ...
            with patch_get_from_config(local_registry_path) as mocked_get_from_config:
                ...
                with open(local_registry.manifest_pk(pkg_name, top_hash).path, encoding='utf8') as fd:
                    pkg = Package.load(fd)
    ...
    def test_read_manifest(self):
        ...
        with open(LOCAL_MANIFEST, encoding='utf8') as fd:
            pkg = Package.load(fd)
        ...
        with open(out_path, encoding='utf8') as fd:
    ...
    def test_package_fetch(self):
        ...
        for dirpath, _, files in os.walk(out_dir):
            for name in files:
                file_count += 1
                with open(os.path.join(dirpath, name), encoding='utf8') as file_:
    ...
    def test_fetch(self):
        ...
        with open(DATA_DIR / 'foo.txt', encoding='utf8') as fd:
            assert fd.read().replace('\n', '') == '123'
        # Copy foo.text to bar.txt
        pkg['foo'].fetch('data/bar.txt')
        with open('data/bar.txt', encoding='utf8') as fd:
            assert fd.read().replace('\n', '') == '123'
    ...
    def test_local_set_dir(self):
        ...
        with open('bar', 'w', encoding='utf8') as fd:
            fd.write(fd.name)
        with open('foo', 'w', encoding='utf8') as fd:
            fd.write(fd.name)
        with open(bazdir / 'baz', 'w', encoding='utf8') as fd:
            fd.write(fd.name)
        with open(foodir / 'bar', 'w', encoding='utf8') as fd:
            fd.write(fd.name)
        ...
        with open('.quiltignore', 'w', encoding='utf8') as fd:
            fd.write('foo\n')
            fd.write('bar')
    ...
    def test_diff(self):
        ...
        # Create a dummy file to add to the package.
        test_file_name = 'bar'
        with open(test_file_name, 'w', encoding='utf8') as fd:
    ...
    def test_commit_message_on_push(self, mocked_workflow_validate):
        ...
        with patch('quilt3.packages.copy_file_list', _mock_copy_file_list), \
            ...
            with open(REMOTE_MANIFEST, encoding='utf8') as fd:
                pkg = Package.load(fd)
    ...
    def test_max_manifest_record_size(self):
        with open(os.devnull, 'wb', encoding='utf8') as buf:
