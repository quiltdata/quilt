...
class GraphQLClientHttpError(GraphQLClientError):
    """HTTP error from GraphQL request."""
...
class GraphQLClientInvalidResponseError(GraphQLClientError):
    """Invalid response format from GraphQL server."""
...
class GraphQLClientGraphQLError(GraphQLClientError):
    """GraphQL error returned from server."""
    def __init__(
        self,
        message: str,
    ...
    def from_dict(cls, error: Dict[str, Any]) -> "GraphQLClientGraphQLError":
        """Create error from GraphQL error dictionary."""
        return cls(
...
class GraphQLClientGraphQLMultiError(GraphQLClientError):
    """Multiple GraphQL errors from server response."""
    def __init__(
        self,
    ...
    def from_errors_dicts(
        ...
        """Create multi-error from list of error dictionaries."""
        return cls(
...
class GraphQLClientInvalidMessageFormat(GraphQLClientError):
    """Invalid message format from GraphQL subscription."""
