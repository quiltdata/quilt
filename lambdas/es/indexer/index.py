...
SELECT_PACKAGE_SIZE = "SELECT SUM(obj['size']) as total_bytes from S3Object obj"
SELECT_PACKAGE_LENGTH = "SELECT COUNT(obj['logical_key']) as total_files from S3Object obj"
...
def select_package_stats(s3_client, bucket, manifest_key) -> str:
    ...
    logger_ = getLogger(LOGGER_NAME)
    try:
        size_response = query_manifest_content(
            s3_client,
            bucket=bucket,
            key=manifest_key,
            sql_stmt=SELECT_PACKAGE_SIZE
        ).read()
        files_response = query_manifest_content(
            s3_client,
            bucket=bucket,
            key=manifest_key,
            sql_stmt=SELECT_PACKAGE_LENGTH
        ).read()
