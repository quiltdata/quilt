...
- [Changed] Thumbnails are now 8-bit everywhere: normalized greyscale montages and Z-projections (multi-channel microscopy stacks, greyscale CZIs) are emitted as 8-bit PNGs (mode `L`) instead of 16-bit (`I;16`) — smaller files, and browsers/the catalog can't display more than 8 bits anyway. Affected thumbnails change slightly: 8-bit tonal steps, and they now resize through the same 8-bit path as every other thumbnail ([#XXXX](https://github.com/quiltdata/quilt/pull/XXXX))
