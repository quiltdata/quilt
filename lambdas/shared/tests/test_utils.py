...
class TestUtils(TestCase):
    ...
    def setUp(self):
        logical_keys = ["foo.csv", "bar/file1.txt", "bar/file2.txt", "bar/baz/file3.txt", "bar/baz/file4.txt"]
        jsonl = ""
        ...
        self.s3response = {
            'Payload': [
                *[{'Records': {'Payload': payload}} for payload in (streambytes, *records_unicode)],
                {'Progress': {'Details': {'BytesScanned': 123, 'BytesProcessed': 123, 'BytesReturned': 123}}},
                {'Stats': {'Details': {'BytesScanned': 123, 'BytesProcessed': 123, 'BytesReturned': 123}}},
                {'End': {}},
            ]
        }
    ...
    def test_separated_env_to_iter(self):
        ...
        with patch.dict(os.environ, {'CONTENT_INDEX_EXTS': ' garbage  gar.bage  '}):
            assert separated_env_to_iter('CONTENT_INDEX_EXTS', predicate=lambda x: x.startswith('.')) == set()
        ...
        with patch.dict(os.environ, {'CONTENT_INDEX_EXTS': ''}):
            assert separated_env_to_iter('CONTENT_INDEX_EXTS') == set(), "Empty string should yield empty set"
        with patch.dict(os.environ, {'CONTENT_INDEX_EXTS': '     '}):
            assert separated_env_to_iter('CONTENT_INDEX_EXTS') == set(), "All spaces should yield empty set"
        with patch.dict(os.environ, {'CONTENT_INDEX_EXTS': '\t\n'}):
            assert separated_env_to_iter('CONTENT_INDEX_EXTS') == set(), "Tab and newline should be empty and falsy"
    ...
    [
        ...
        pytest.param(
            ...
            marks=pytest.mark.xfail(
                raises=AssertionError, reason="unclear but logger from @logger doesn't get patched?"
            ),
        ),
    ],
)
